...
async def fetch_leaderboard_task():
    ...
    try:
        ...
        import json
        import asyncio
        platform.window.fetchLeaderboardJS()
        
        # Poll até o JS terminar a requisição
        while str(platform.window.leaderboardStatus) == "loading":
            await asyncio.sleep(0.1)
            
        status = str(platform.window.leaderboardStatus)
        if status == "loaded":
            text = str(platform.window.leaderboardData)
...
def submit_score():
    try:
        import platform
        platform.window.postScore(player_name, score, lives, insignias_decomposicao, insignias_abstracao)
        print(f"Score enviado (via JS): {player_name} = {score} pts")
    except Exception as e:
...
def submit_metrics():
    try:
        ...
        platform.window.postMetrics(json.dumps(payload))
        print(f"Métricas enviadas para {player_name}")
    except Exception as e:
